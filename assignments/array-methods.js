...
// Problem 1: They want to give each of the runners a tag with their last name and id number so that spotters can easily identify everyone. Create a new array named callTags and populate it with objects containing the runner's information.
let callTags = [];
runners.forEach(runner => callTags.push({last_name: runner.last_name, id: runner.id}));
console.log(callTags);

// Problem 2: End of the year is coming up and the Community Center needs to send out donation records to each company that has donated funds. To get these records out they have decided to get a list of each event's records and filter out the relevant information so that they can later create itemized listings. Create an array called donationRecords and a function called getRecords to fill the array.
let donationRecords = [];
runners.forEach(runner => 
    donationRecords.push({company_name: runner.company_name, donation: runner.donation}));
console.log(donationRecords);

// Problem 3: The community center is planning to host another event in the near future and wants to notify each of the representatives. Create an array contactList containing the email address and full name of each participant.
let contactList = [];
runners.forEach(runner =>
    contactList.push({full_name: `${runner.first_name} ${runner.last_name}`, email: runner.email}));
